...
namespace defines {
    ...
    static flinearcolor Invisible_ESPColor{ 1.f,0.f,0.f,1.f };//BoxInvisible

    static flinearcolor VisibleSkeletonColor{ 0.0f,255.0f,0.0f,1.0f };//VisibleSkeletonColor
    static flinearcolor InvisibleSkeletonColor{ 1.f,0.f,0.f,1.f };//VisibleSkeletonColor

    static flinearcolor InvisibleSnapColor{ 1.f,0.f,0.f,1.f };
...
/*
 ...
 std::array<std::wstring, 3> bone_types = {  L"head",  L"chest",L"leg" };
    std::array<std::wstring, 3> box_types = { L"corner", L"2d", L"3d" };
    ...
    void hk_draw_menu(uobject* font, bool open_canvas, ucanvas* canvas) {
        ...
        if (canvas && font && open_canvas) {
            ...
            draw_head(canvas, font, L"VIP - Menu: F2", { 0.0f, 1.5f, 1.0f, 1.0f }, { 145 + vertical_padding, 40 + horizontal_padding });
            canvas->k2_drawline({ 145 + linex_padding, 57 + liney_padding }, { 145 + linex_padding + menu_width, 57 + liney_padding },
                border_thickness, border_color);

            process_input();

            draw_text(canvas, font, L"Visuals:", rainbow_color, { 145 + vertical_padding, 70 + horizontal_padding });

            draw_text(canvas, font, globals::visuals::headbox ? L"headbox: on" : L"headbox: off",
                (current_selection == 3) ? flinearcolor{ 1, 1, 0, 1 } : get_color(globals::visuals::headbox), { 145 + vertical_padding, 90 + horizontal_padding });

            draw_text(canvas, font, globals::visuals::skeleton ? L"skeleton: on" : L"skeleton: off",
                (current_selection == 4) ? flinearcolor{ 1, 1, 0, 1 } : get_color(globals::visuals::skeleton), { 145 + vertical_padding, 110 + horizontal_padding });

            draw_text(canvas, font, globals::visuals::snapline ? L"snapline: on" : L"snapline: off",
                (current_selection == 5) ? flinearcolor{ 1, 1, 0, 1 } : get_color(globals::visuals::snapline), { 145 + vertical_padding, 130 + horizontal_padding });

            draw_text(canvas, font, globals::visuals::healthbar ? L"healthbar: on" : L"healthbar: off",
            ...
            draw_text(canvas, font, L"Aimbot:", rainbow_color, { 145 + vertical_padding, 180 + horizontal_padding });

            draw_text(canvas, font, globals::aimbot::aimbot ? L"enable: on" : L"enable: off",
                (current_selection == 7) ? flinearcolor{ 1, 1, 0, 1 } : get_color(globals::aimbot::aimbot), { 145 + vertical_padding, 200 + horizontal_padding });

            draw_text(canvas, font, globals::aimbot::vish_check ? L"visible check: on" : L"visible check: off",
                (current_selection == 8) ? flinearcolor{ 1, 1, 0, 1 } : get_color(globals::aimbot::vish_check), { 145 + vertical_padding, 220 + horizontal_padding });

            draw_text(canvas, font, globals::aimbot::recoil_control ? L"recoil control: on" : L"recoil control: off",
            ...
            draw_text(canvas, font, std::wstring(L"smooth: " + std::to_wstring(globals::aimbot::aim_smooth)).c_str(),
                (current_selection == 10) ? flinearcolor{ 1, 1, 0, 1 } : flinearcolor{ 1, 1, 1, 1 }, { 145 + vertical_padding, 260 + horizontal_padding });

            draw_text(canvas, font, std::wstring(L"fov: " + std::to_wstring(globals::aimbot::aim_fov)).c_str(),
                (current_selection == 11) ? flinearcolor{ 1, 1, 0, 1 } : flinearcolor{ 1, 1, 1, 1 }, { 145 + vertical_padding, 280 + horizontal_padding });

            draw_text(canvas, font, std::wstring(L"bone: " + bone_types[globals::aimbot::aim_bone]).c_str(),
                (current_selection == 12) ? flinearcolor{ 1, 1, 0, 1 } : flinearcolor{ 1, 1, 1, 1 }, { 145 + vertical_padding, 300 + horizontal_padding });

            draw_text(canvas, font, std::wstring(L"key: " + key_types[globals::aimbot::aim_key]).c_str(),
            ...
            draw_text(canvas, font, L"Misc:", rainbow_color, { 145 + vertical_padding, 360 + horizontal_padding });

            draw_text(canvas, font, globals::misc::skiptutorial ? L"skip tutorial: on" : L"skip tutorial: off",
            ...
            draw_text(canvas, font, globals::misc::skin_changer ? L"unlock skins: on" : L"unlock skins: off",
