...
// SAFE SPOOF CALL MACRO with NULL checks
#define SAFE_SPOOF_CALL(func, ...) \
    do { \
        auto _func_ptr = func; \
        if (_func_ptr) { \
            __try { \
                SPOOF_CALL(_func_ptr)(__VA_ARGS__); \
            } \
            __except(EXCEPTION_EXECUTE_HANDLER) { \
                /* Silently handle exceptions from bad calls */ \
            } \
        } \
    } while(0)

// SAFE SPOOF CALL for skin changer with parameter validation
#define SAFE_SKIN_SPOOF_CALL(func, equippable, skin_data, chroma, ...) \
    do { \
        if (equippable && skin_data) { \
            SAFE_SPOOF_CALL(func, equippable, skin_data, chroma, __VA_ARGS__); \
        } \
    } while(0)
